...
<div>
    <form wire:submit="resetPassword" class="space-y-4">
        <div>
            ...
            <x-text-input wire:model="email" id="email" variant="auth" class="block mt-1.5 w-full" type="email" name="email" required autofocus autocomplete="username" />
            <x-input-error :messages="$errors->get('email')" class="mt-2" />
        </div>

        <div>
            <x-input-label for="password" :value="__('app.auth.password_label')" variant="auth" />
            <x-text-input wire:model="password" id="password" variant="auth" class="block mt-1.5 w-full" type="password" name="password" required autocomplete="new-password" />
